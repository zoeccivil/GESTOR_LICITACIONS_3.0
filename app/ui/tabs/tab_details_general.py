...
class TabDetailsGeneral(QWidget):
    ...
    def load_data(self):
        ...
        print(f"TabDetailsGeneral: Cargando datos para {self.licitacion.numero_proceso}")
        self._widgets['codigo'].setText(self.licitacion.numero_proceso)
        self._widgets['nombre'].setText(self.licitacion.nombre_proceso)
        ...
        try:
            ...
            if self.licitacion.institucion in nombres_instituciones:
            ...
            if self.licitacion.institucion: # Mostrar la que está guardada aunque falle la carga
                combo_inst.addItem(self.licitacion.institucion)
                combo_inst.setCurrentText(self.licitacion.institucion)
        combo_inst.blockSignals(False)


        # Actualizar display de empresas
        ...
        combo_adj.clear()
        nuestras_empresas_nombres = {str(e) for e in self.licitacion.empresas_nuestras}
        competidores_nombres = {o.nombre for o in self.licitacion.oferentes_participantes}
        ...
        if self.licitacion.adjudicada_a and self.licitacion.adjudicada_a in todos_participantes:
            combo_adj.setCurrentText(self.licitacion.adjudicada_a)
        else:
             combo_adj.setCurrentText(self.licitacion.adjudicada_a or "") # Permitir texto libre
        combo_adj.blockSignals(False)
        self._on_estado_change() # Habilita/deshabilita adjudicado_a

        # Cargar Checkboxes y Texto
        self._widgets['progreso_label'].setText(f"{self.licitacion.get_porcentaje_completado():.1f}%")
        self._widgets['docs_manual_check'].setChecked(self.licitacion.docs_completos_manual)
        self._widgets['fase_b_check'].setChecked(self.licitacion.fase_B_superada)
        self._widgets['motivo_text'].setPlainText(self.licitacion.motivo_descalificacion)
            ...
            if fecha_str:
                try:
                    ...
                    if qdate.isValid():
                        ...
                        date_edit.clear() # Limpiar si la fecha no es válida
                except Exception:
                    date_edit.clear()
            else:
                date_edit.clear() # Limpiar si no hay fecha
    ...
    def collect_data(self) -> bool:
        ...
        try:
            ...
            # Validación básica (nombre y código no vacíos)
            if not self.licitacion.numero_proceso or not self.licitacion.nombre_proceso:
                QMessageBox.warning(self, "Datos Incompletos", "El Código y el Nombre del Proceso no pueden estar vacíos.")
                return False

            print("TabDetailsGeneral: Datos recolectados.")
            return True # Indicar éxito
        except Exception as e:
            QMessageBox.critical(self, "Error al Recolectar", f"No se pudieron leer los datos de la pestaña General:\n{e}")
    ...
    def _abrir_selector_empresas(self):
        ...
        try:
            ...
            nombres_actuales = {str(e) for e in self.licitacion.empresas_nuestras}
            ...
            if dialogo.exec() == QDialog.DialogCode.Accepted:
                ...
                self._actualizar_display_empresas()
                # Opcional: Podrías necesitar notificar a otras pestañas (ej. Lotes, Competidores)
                # self.parent_window.tab_lotes.actualizar_combos_empresa()
                # self.parent_window.tab_competitors.actualizar_opciones_ganador()
        except Exception as e:
    ...
    def _actualizar_display_empresas(self):
        ...
        if not nombres:
            ...
            self._widgets['empresas_label'].setStyleSheet("color: red;")
        else:
            texto = ", ".join(sorted(nombres))
            self._widgets['empresas_label'].setStyleSheet("color: black;") # Color normal

        self._widgets['empresas_label'].setText(texto)

    # --- Métodos para acciones de documentos (adaptados de Tkinter) ---
    # Estos métodos ahora abren diálogos PyQt6 o realizan acciones directas
